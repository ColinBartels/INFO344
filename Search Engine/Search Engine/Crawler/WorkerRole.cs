...
namespace WorkerRole1
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        {
            //Encodes URL
            byte[] encodedTitle = new UTF8Encoding().GetBytes(title);
            byte[] hash = ((HashAlgorithm)CryptoConfig.CreateFromName("MD5")).ComputeHash(encodedTitle);
            string encoded = BitConverter.ToString(hash)
               .Replace("-", string.Empty)
               .ToLower();

            //Adds new page to table
            Page page = new Page(title, url, date, encoded);
